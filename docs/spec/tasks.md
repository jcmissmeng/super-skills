...
| ID | 任务 | 验收标准 | 优先级 |
|----|------|----------|--------|
| T4 | 修复 project-onboard 测试配置 | conftest.py 不再引用不存在的 scripts/ 目录，tests 可正常运行 | P1 |
| T5 | 补充项目根目录 .gitignore | 根目录有 .gitignore，排除 node_modules/.env 等 | P1 |

## 待开发 (pending)

| ID | 任务 | 验收标准 | 优先级 | 来源 |
|----|------|----------|--------|------|
| T6 | 补充项目根目录 README.md | 包含项目定位、技能列表、使用方式说明 | P0 | 项目必备 |
| T7 | 修复 project-onboard 缺失的 .gitignore | skills/project-onboard/.gitignore 存在且内容合理 | P2 | xcopy 遗漏 |
